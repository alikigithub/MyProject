...
export const getHomeUsers = createAsyncThunk('gethomeuser/chat', async () => {
  try {
    ...
    if (!dataofUsers.length) {
      return [];
    }

    const usersSnapshot = await firestore()
      .collection('Users')
      .where(firestore.FieldPath.documentId(), 'in', dataofUsers)
      .get();
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(getHomeUsers.fulfilled, (state, action) => {
        state.homedataofUsers = action.payload?.map(user => ({
          id: user.id,
          homeContact: (user as UserData).homeContact || [],
          status: (user as UserData).status || '',
          contact: (user as UserData).contact || [],
          profilePic: (user as UserData).profilePic || '',
          UserName: (user as UserData).UserName || '',
